  ...
  return cp
    ...
    .on('close', code => {
      if (code === 0) {
        ...
        console.warn(
          `Jekyll build failed with code ${code}. This might be expected if Jekyll is not installed.`
        );
        done();
      }
    })
...
// Copy CSS files
function copyCSS() {
  return src('assets/css/**/*.css').pipe(dest('_site/assets/css')).pipe(browserSync.stream());
}

// Copy JavaScript files
function copyJS() {
  return src('assets/js/**/*.js').pipe(dest('_site/assets/js')).pipe(browserSync.stream());
}

// Copy images
function copyImages() {
  return src('assets/img/**/*').pipe(dest('_site/assets/img')).pipe(browserSync.stream());
}
